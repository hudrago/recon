...
import type { Refund, ReturnRecord } from "@recon/domain";
import {
...
describe("ExceptionsController (http)", () => {
  ...
  beforeEach(async () => {
    getSession = vi.fn().mockResolvedValue({
      user: { id: "user_1", email: "operator@example.com" },
    });
    ...
    const moduleRef = await Test.createTestingModule({ imports: [AppModule] })
      ...
      .overrideProvider(RESTOCK_GATEWAY)
      ...
      .useValue({
        getRole: async (_userId: string, orgId: string) =>
          orgId === "org_1" ? "operator" : null,
      })
      .compile();
  ...
  it("fetches a single exception scoped to the requesting org", async () => {
    ...
    await request(app.getHttpServer())
      .get(`/orgs/org_2/exceptions/${exception.id}`)
      .expect(403);
  });

  it("returns 404 for a nonexistent exception id", async () => {
    await request(app.getHttpServer())
      .get("/orgs/org_1/exceptions/does-not-exist")
      .expect(404);
  });
  ...
  it("approves then executes a refund, and rejects a cross-org attempt", async () => {
    ...
    expect(response.body.refundId).toBe(
      "refund_refund:REFUND_MISSING:org_1:ret_http_1",
    );
    expect(
      (await exceptions.getAuditLog("org_1")).map((entry) => entry.actor),
    ).toEqual(["user_1", "user_1"]);
  });

  it("exposes a redacted, exception-scoped audit timeline and rejects a cross-org attempt", async () => {
    await exceptions.ingestReturn(returnRecord, [], now);
    const [exception] = await exceptions.listOpenExceptions("org_1");
    ...
    await exceptions.executeRefund({ exceptionId: exception.id }, "user_1");

    await request(app.getHttpServer())
      .get(`/orgs/org_2/exceptions/${exception.id}/audit`)
      .expect(403);
    ...
    expect(response.body[0]).not.toHaveProperty("before");
    expect(response.body[0]).not.toHaveProperty("after");
  });

  it("approves then executes a restock", async () => {
    const refund: Refund = {
      id: "rf_http_1",
      orgId: "org_1",
      orderId: "order_1",
    ...
    await exceptions.ingestRefund(
      refund,
      [],
      new Date(
        new Date(refund.issuedAt).getTime() +
          RESTOCK_MISSING_THRESHOLD_MS +
          1000,
      ),
    );
    const [exception] = (await exceptions.listOpenExceptions("org_1")).filter(
      (exception) => exception.code === "RESTOCK_MISSING",
    );

    await request(app.getHttpServer())
      .post(`/orgs/org_1/exceptions/${exception.id}/approve`)
      .send({ reason: "Restock approved", quantity: 2 })
      .expect(200);

    const response = await request(app.getHttpServer())
      .post(`/orgs/org_1/exceptions/${exception.id}/actions/restock`)
      .send({})
      .expect(200);

    expect(response.body.adjustmentId).toBe(
      `adjustment_restock:${exception.id}`,
    );
  });

  it("approves then executes an invoice issuance, with no approval terms required", async () => {
    const order = {
      id: "order_http_1",
      orgId: "org_1",
      currency: "EUR",
    ...
    await request(app.getHttpServer())
      .post(`/orgs/org_1/exceptions/${exception.id}/approve`)
      .send({ reason: "Invoice issuance approved" })
      .expect(200);

    const response = await request(app.getHttpServer())
      .post(`/orgs/org_1/exceptions/${exception.id}/actions/invoice`)
      .send({})
      .expect(200);
