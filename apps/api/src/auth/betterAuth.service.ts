    ...
    this.auth = betterAuth({
      ...
      emailVerification: {
        sendVerificationEmail: async ({ user, url }) => {
          await mailer.send({
            to: user.email,
            subject: "Confirme o seu email — Recon",
            html: `<p>Confirme o seu email para continuar: <a href="${url}">${url}</a></p>`,
          });
        },
      },
      hooks: {
          ...
          beforeDelete: async (user) => {
            ...
            if (
              ...
              throw new APIError("CONFLICT", {
                message:
                  "Delete or transfer owned organizations before deleting the account.",
              });
            }
            if (invitations > 0)
              throw new APIError("CONFLICT", {
                message:
                  "Revoke outstanding organization invitations before deleting the account.",
              });
          },
        },
      },
      ...
      plugins: [
        organization({
          ...
          // Best-effort field shape from better-auth's organization plugin source (data.id,
          // data.email, data.organization.name) — not from published docs, verify if it changes.
          sendInvitationEmail: async (data) => {
            const acceptUrl = `${webUrl}/accept-invitation/${data.id}`;
            await mailer.send({
              to: data.email,
              subject: `Foi convidado para ${data.organization.name} na Recon`,
              html: `<p>Foi convidado para se juntar a <strong>${data.organization.name}</strong> na Recon.</p><p><a href="${acceptUrl}">Aceitar convite</a></p>`,
            });
          },
          organizationHooks: {
