...
import type { DomainException, ExceptionCode, ExceptionStatus, Refund } from '@recon/domain';
...
export class PrismaExceptionStore implements ExceptionStore {
  ...
  async claimWebhook(provider: string, eventId: string, orgId: string): Promise<boolean> {
    try {
      await this.prisma.webhookReceipt.create({ data: { provider, eventId, orgId, status: 'PROCESSING' } });
      return true;
    } catch (error) {
      if (typeof error === 'object' && error !== null && 'code' in error && error.code === 'P2002') {
        const reclaimed = await this.prisma.webhookReceipt.updateMany({
          where: { provider, eventId, orgId, status: 'PROCESSING', updatedAt: { lte: new Date(Date.now() - 5 * 60 * 1000) } },
  ...
  async completeWebhook(provider: string, eventId: string): Promise<void> {
    await this.prisma.webhookReceipt.update({ where: { provider_eventId: { provider, eventId } }, data: { status: 'SUCCEEDED' } });
  }
  ...
  async saveWithAudit(exception: DomainException, entry: AuditLogEntry): Promise<void> {
    await this.prisma.$transaction([
      this.prisma.exceptionRecord.upsert({
        ...
        update: { status: exception.status, context: exception.context as object },
      }),
  ...
  async saveRefund(refund: Refund): Promise<void> {
    await this.prisma.$transaction([
      this.prisma.refundRecord.upsert({
        ...
        update: { orderId: refund.orderId, amount: refund.amount, currency: refund.currency, issuedAt: new Date(refund.issuedAt) },
      }),
  ...
  async listRefunds(orgId: string, orderId: string): Promise<Refund[]> {
    const records = await this.prisma.refundRecord.findMany({ where: { orgId, orderId } });
    return records.map((record) => ({ ...record, issuedAt: record.issuedAt.toISOString() }));
  }
  ...
  async cancelPendingRefundEvaluation(orgId: string, orderId: string): Promise<void> {
    ...
    const ids = records
      .filter((record) => {
        ...
        return evaluation.kind === 'refund-missing' && evaluation.returnRecord.orgId === orgId && evaluation.returnRecord.orderId === orderId;
      })
      .map((record) => record.id);
    if (ids.length > 0) await this.prisma.pendingEvaluation.deleteMany({ where: { id: { in: ids } } });
  }

  async findRefundMissing(orgId: string, orderId: string): Promise<DomainException | undefined> {
    const record = await this.prisma.exceptionRecord.findFirst({ where: { orgId, orderId, code: 'REFUND_MISSING' } });
    return record ? toDomainException(record) : undefined;
  }

  async claimAction(idempotencyKey: string, exceptionId: string, orgId: string, orderId: string): Promise<ActionClaim> {
    try {
      await this.prisma.executedAction.create({ data: { idempotencyKey, exceptionId, orgId, orderId, actionKind: 'REFUND', status: 'PENDING' } });
    ...
    const retried = await this.prisma.executedAction.updateMany({
      where: {
        ...
        OR: [
          { status: 'FAILED' },
          { status: 'PENDING', updatedAt: { lte: new Date(Date.now() - 5 * 60 * 1000) } },
        ],
      },
    ...
    const action = await this.prisma.executedAction.findUnique({ where: { idempotencyKey } });
    ...
    if (action?.status === 'SUCCEEDED' && action.result) {
      return { status: 'succeeded', result: action.result as unknown as ExecutedActionResult };
    }
    const reserved = await this.prisma.executedAction.findFirst({ where: { orgId, orderId, actionKind: 'REFUND' } });
    if (reserved?.status === 'SUCCEEDED' && reserved.result) {
      return { status: 'succeeded', result: reserved.result as unknown as ExecutedActionResult };
    }
  ...
  async completeAction(idempotencyKey: string, result: ExecutedActionResult, refund: Refund, exception: DomainException, entry: AuditLogEntry): Promise<void> {
    await this.prisma.$transaction([
      ...
      this.prisma.exceptionRecord.update({
        where: { id: exception.id },
        data: { status: exception.status, context: exception.context as object },
      }),
      this.prisma.refundRecord.upsert({
        where: { orgId_id: { orgId: refund.orgId, id: refund.id } },
        create: { ...refund, issuedAt: new Date(refund.issuedAt) },
